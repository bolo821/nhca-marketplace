      ...
      <div className={`container-fluid wide-${window.location.pathname.split("/")[2] === "invest" ? "lg" : "xl"}`}>
        <div className="nk-header-wrap">
          ...
          <div className="nk-header-tools">
            {
              active ?
                <Button variant="contained" onClick={() => onConnectWallet()}>{account.substring(0, 3)} ... {account.substring(account.length - 3)}</Button>
                :
                <Button variant="contained" onClick={() => onConnectWallet()}>Connect Metamask</Button>
            }
